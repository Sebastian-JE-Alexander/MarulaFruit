...
def find_misclassified(val_dir=config.VAL_DIR,
                       weights_path=config.MODEL_WEIGHTS_PATH,
                       classes_path=config.CLASSES_PATH,
                       output_dir=config.MISCLASSIFIED_DIR):
    ...
    assert dataset.classes == classes, (
        f"Validation folder classes {dataset.classes} don't match "
        f"outputs/classes.txt {classes} - retrain, or check dataset/validation/ "
    ...
    # a prior model don't linger and get confused with this run's
        ...
        for idx in range(len(dataset)):
            ...
            if pred_idx != true_idx:
                ...
                error_folder = os.path.join(
                    output_dir, f"true_{true_class}_predicted_{pred_class}")
                os.makedirs(error_folder, exist_ok=True)

                fname = os.path.basename(path)
                # confidence prefix so files sort with the model's most
                # confident (and most concerning) mistakes first
                dest = os.path.join(error_folder, f"{confidence:.0%}_{fname}")
                shutil.copy(path, dest)

                results.append({"path": path, "true": true_class,
                                "predicted": pred_class, "confidence": confidence})

    print(f"Checked {len(dataset)} validation images, found {len(results)} misclassified.\n")

    error_counts = Counter((r["true"], r["predicted"]) for r in results)
    for (true_c, pred_c), count in sorted(error_counts.items(), key=lambda kv: -kv[1]):
        print(f"  {count} image(s): true={true_c} -> predicted={pred_c}")

    if results:
        print(f"\nCopies saved to {output_dir}/true_<X>_predicted_<Y>/")
        print("Filenames are prefixed with the model's confidence in its "
              "(wrong) prediction - look at the highest-confidence mistakes "
              "first, they're the most informative: either a genuinely "
              "hard/ambiguous case, or a labelling issue worth double-checking.")
